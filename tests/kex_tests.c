...
{ // Testing key exchange
    ...
    Status = KeyGeneration_A(PrivateKeyA, PublicKeyA, CurveIsogeny, true, 0);                     // Get some value as Alice's secret key and compute Alice's public key
    if (Status != CRYPTO_SUCCESS) {                                                  
        goto cleanup;
    }  
...
{ // Benchmarking key exchange
    ...
    {
        cycles1 = cpucycles();
        Status = KeyGeneration_A(PrivateKeyA, PublicKeyA, CurveIsogeny, true, 0);                     
        if (Status != CRYPTO_SUCCESS) {                                                  
            passed = false;
            break;
        }    
...
void *verify_thread(void *TPV) {
	...
	while (1) {
		...
		if (bit == 0) {
			...
			/*
			if (verifyBatchA->batchSize + verifyBatchC->batchSize != batchSize) {
				sem_wait(&verifyBatchC->sign_sem);
			} else {
				for (i = 0; i < verifyBatchC->batchSize - 1; i++) {
					sem_post(&verifyBatchC->sign_sem);			
				}
			}*/
